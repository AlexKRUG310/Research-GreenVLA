...
def train(cfg: DictConfig):
    ...
    # Прямая инициализация политики для избежания конфликтов аргументов Hydra
    from lerobot.common.policies.greenvla_policy.modeling_greenvla_policy import GreenVLAPolicy
    policy = GreenVLAPolicy(config=policy_config)
    ...
    num_frames = getattr(robotics_dataset, 'num_samples', len(robotics_dataset))
    num_episodes = getattr(robotics_dataset, 'num_episodes', 0)
