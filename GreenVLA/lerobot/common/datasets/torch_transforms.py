...
class TokenizeGreenVLAInputsTransform:
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        if not isinstance(prompt, str):
            prompt = prompt.item()
        # FIX: Robust retrieval for state and image_mask
        state = data.get("state", data.get("observation.state", data.get("observation/state")))
        # Default image_mask: 1 for each camera key (indicates image is present)
        default_image_mask = {"egocentric": 1}
        image_mask = data.get("image_mask", default_image_mask)
...
class TorchTransformedDataset(TorchDataset[SampleType]):
    ...
    def get_dataset_summary(self) -> dict[str, Any]:
        summary_info = {}
        try:
            summary_info["dataset_id"] = getattr(self, "_dataset_id", "local_dataset")
            summary_info["num_samples"] = getattr(self, "_num_samples", len(self))
            summary_info["num_episodes"] = getattr(self, "_num_episodes", 0)
            summary_info["dataset_length"] = getattr(self, "_num_samples", len(self)) / getattr(self, "_fps", 30.0)
...
class ResizeImagesTorch:
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        transformed_images = {}
        # FIX: Support both 'image' and 'observation/image' keys, and handle if it's a single Tensor
        img_data = data.get("image", data.get("observation/image", {}))
        
        # If it's a single tensor, wrap it in a dict with the expected key 'egocentric'
        if isinstance(img_data, torch.Tensor):
            img_data = {"egocentric": img_data}
        elif not isinstance(img_data, dict):
            img_data = {}
            
        for k, v_img_tensor in img_data.items():
