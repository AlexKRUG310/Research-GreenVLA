...
def compute_dataset_stats_from_dataset(dataset, max_samples=100000):
    try:
        dataset_name = dataset.get_dataset_summary().get("dataset_id", "local_dataset")
    except Exception:
    ...
    try:
        ...
        return_fake_images = False
    try:
        dataloader.dataset._dataset._dataset.return_fake_images = True
    except AttributeError:
        pass  # Игнорируем, если атрибут недоступен
    ...
    try:
        ...
        pass  # Игнорируем, если атрибут недоступен (например, у LeRobotDataset нет _dataset)
    return norm_stats
